...
class BatchProducerIT extends WordSpec with Matchers with SharedSparkSession {
  ...
  "BatchProducer.processOneBatch" should {
    ...
    "filter and save a batch of  transaction, wait 59 min, fetch the next batch" in withTempDir { tmpDir =>

      // Timer simulate a clock
      ...
      val end0 = Instant.parse("2018-08-02T00:59:00Z")
      ...
      val savedTransactions = spark
        .read
        .parquet(tmpDir.toString)
        .as[Transaction]
        .collect()
