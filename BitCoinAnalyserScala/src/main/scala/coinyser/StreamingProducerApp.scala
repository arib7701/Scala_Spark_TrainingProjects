...
object StreamingProducerApp extends IOApp {
  ...
  def run(args: List[String]): IO[ExitCode] = {
    ...
    /*
      subscribe start a background thread and complete immediately when we run it
      but we need to keep running forever
      we use flatmap that return IO.never to keep the main thread running until we manually kill the process
     */
